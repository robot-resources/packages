/**
 * MCP server for scraper
 * Exposes scraper_compress_url and scraper_crawl_url as MCP tools
 */
...
export async function compressUrl({
  ...
  try {
    ...
    pushDebugEntry('scraper-compressions', {
      url,
      mode: mode ?? 'auto',
      tokens_original: originalTokens,
      tokens_compressed: convertResult.tokenCount,
