...
describe('OpenClaw harness: provider registration', () => {
  ...
  beforeEach(async () => {
    vi.resetModules();
    // Stub the local HTTP server so the harness doesn't actually bind a port.
  ...
  it('registers the robot-resources provider with the OC SDK', () => {
    const { api, registrations } = createMockOpenClawApi();

    plugin.register(api);

    expect(registrations.providers).toHaveLength(1);
    expect(registrations.providers[0].id).toBe('robot-resources');
  });

  it('registers in subscription mode too', () => {
    const { api, registrations } = createMockOpenClawApi({
  ...
  it('provider has both catalog and staticCatalog with run callbacks', () => {
    const { api, registrations } = createMockOpenClawApi();

    plugin.register(api);

    const provider = registrations.providers[0];
    expect(typeof provider.catalog?.run).toBe('function');
    expect(typeof provider.staticCatalog?.run).toBe('function');
  });
